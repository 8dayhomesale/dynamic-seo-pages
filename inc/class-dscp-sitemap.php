...
class DSCP_Sitemap_Provider implements WPSEO_Sitemap_Provider {
	...
	public function get_index_links( $max_entries ) {
		...
		$index[] = array(
			'loc'     => WPSEO_Sitemaps_Router::get_base_url( 'dscp-sitemap.xml' ),
			'lastmod' => date( 'c' ),
		);
	...
	public function get_sitemap_links( $type, $max_entries, $current_page ) {
		...
		foreach ( $sub_pages as $sub_page_id ) {
			$sitemap_pages = get_post_meta( $sub_page_id, 'dscp_sitemap_pages', true );
			$base_page_id = get_post_meta( $sub_page_id, 'dscp_base_page', true );

			if ( empty( $sitemap_pages ) || empty( $base_page_id ) ) {
				continue;
			}
			...
			foreach ( $sitemap_pages as $sitemap_page ) {
				$links[] = array(
					'loc' => home_url( $base_page->post_name . $sitemap_page ),
